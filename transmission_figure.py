...
# Example Call
# python transmission_figure.py --file_path /home/sbechtel/Documents/DEIMOS_Light_Echo/Targets/J1630B/final_redux/setup_FWHM/Science_coadd/ --exten 67 --redshift 4.601 --qso J1630
...
if qso.lower() == 'j1630':
    ...
    z_qso = 4.709
    qso_wave = lya * (1 + z_qso)
    #qso_sig = 50  # km/s from sdss skyserver
    qso_sig = 400 # Just copying J1630 uncertainty
else:
...
cont_fit = curve_fit(cont_powerlaw, x, y, maxfev=100000,
                     bounds=([0, -np.inf, 1300 * (1 + z) - 1], [np.inf, 0, 1300 * (1 + z) + 1]),
                     sigma=y_sig)
...
simulated_comov, simulated_trans = trans_sig_comp(comov_range[trans_mask], trans_flux[trans_mask],
                                                  trans_sig[trans_mask],
...
ax.plot(vel_range[simul_mask], np.median(simulated_trans_full, axis=0), 'g--', linewidth=3,
        label=r'\textbf{Median of Simulated Transmission}')
ax.plot(vel_range[simul_mask], simulated_16p, color='orange', linestyle=':', linewidth=3,
        label=r'\textbf{16th/84th Percentile of Simulated Transmission}')
